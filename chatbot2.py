...
def main() -> None:
    ...
    if file_results:
        ...
        for file_result in file_results:
            ...
            match_result = file_result.get("match") or {}
            catalog = file_result.get("catalog") or {}
            detailed_lines.append(f"Details: {filename}")
            detailed_lines.append(
                "Type: "
                f"{format_report_value(file_result.get('document_type'))} "
                f"(source: {format_report_value(file_result.get('type_source'))})"
            )
            detailed_lines.append(
                "Type detail: "
            ...
            detailed_lines.append(
                "File info: "
                f"format={format_report_value(file_format)}, "
                f"size_bytes={format_report_value(file_size_bytes)}, "
                f"processing_status={format_report_value(processing_status)}, "
                f"is_scanned={format_report_value(is_scanned)}"
            )
            detailed_lines.append(
                "Validation: "
                f"{format_report_value(validation.get('status'))} - "
                f"{format_report_value(validation.get('reason'))}"
            )
            if file_result.get("has_error") is not None:
                ...
                if file_result.get("error_reasons"):
                    detailed_lines.append("Error reasons:")
                    detailed_lines.append(
                        format_json_block(file_result.get("error_reasons"))
                    )
            if catalog:
                detailed_lines.append("Catalog info:")
                detailed_lines.append(
                    f"  Customer: {format_report_value(catalog.get('customer'))}"
                )
                detailed_lines.append(
                    f"  Description: {format_report_value(catalog.get('description'))}"
                )
                detailed_lines.append(
                    "  Expected extensions: "
                    f"{format_report_value(', '.join(catalog.get('expected_extensions', []) or []))}"
                )
                detailed_lines.append(
                    f"  Match status: {format_report_value(catalog.get('match_status'))}"
                )
                detailed_lines.append(
                    f"  Type mismatch: {format_report_value(catalog.get('type_mismatch'))}"
                )
            if file_result.get("extraction_warning"):
                detailed_lines.append(
                    f"Extraction warning: {format_report_value(file_result.get('extraction_warning'))}"
                )
            if file_result.get("extraction_error"):
                detailed_lines.append(
                    f"Extraction error: {format_report_value(file_result.get('extraction_error'))}"
                )
            if file_result.get("text_extraction_error"):
                detailed_lines.append(
                    "Text extraction error: "
                    f"{format_report_value(file_result.get('text_extraction_error'))}"
                )
            if file_result.get("ocr_error"):
                detailed_lines.append(
                    f"OCR error: {format_report_value(file_result.get('ocr_error'))}"
                )
            if file_result.get("ocr_used"):
                detailed_lines.append("OCR was used to extract text for this file.")
            if validation.get("issues"):
                detailed_lines.append("Validation issues:")
                detailed_lines.append(format_json_block(validation.get("issues")))
            if match_result:
                detailed_lines.append("Dataset match:")
                detailed_lines.append(
                    f"{format_report_value(match_result.get('status'))}: "
                    f"{format_report_value(match_result.get('reason'))} "
                    f"(confidence {format_report_value(match_result.get('confidence'))})"
                )
            ...
            if file_result.get("keyword_result"):
                detailed_lines.append("Keyword classification:")
