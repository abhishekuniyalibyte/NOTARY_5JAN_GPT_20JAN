...
def call_openai_extraction(
    ...
    try:
        ...
        return {"status": "error", "message": f"OpenAI request failed: {exc}"}
...
def process_collection_with_llm(
    ...
    for document in collection.documents:
        ...
        if llm_settings.get("enabled") and raw_text:
            llm_payload = call_openai_extraction(
...
def run_flow(
    ...
    for file_info in uploaded_files:
        ...
        if llm_settings.get("enabled"):
            if doc_text:
                llm_result = call_openai_classification(
...
def main() -> None:
    ...
    enable_llm = st.sidebar.checkbox("Enable LLM extraction + classification (OpenAI)", value=True)
    ...
    if enable_llm:
        ...
        if openai_api_key:
            ...
            st.sidebar.warning("OPENAI API key not found in .env")
    else:
        openai_api_key = ""
            ...
            with st.spinner("Analyzing documents and generating reports..."):
                ...
                if results and "file_results" in results:
                    ...
                    for fr in results["file_results"]:
                        ...
                        # Detailed entry (omitted for brevity here used identical logic to old one)
    ...
    if not file_results and not certificate_text:
        ...
        if submitted:
            ...
            qa_result = call_openai_document_qa(
                model=qa_model,
                api_key=openai_api_key,
                question=question,
                context=context,
